...
def run_simulations(network_id=1):
    print("Dynamically calculating unscaled echelon bullwhip ratios for Log Scale plot...")
    
    config = get_willems_config(network_id)
    eval_demands = get_deterministic_demands(network_id=network_id, steps=100, seed=42)
    eval_demands_scaled = eval_demands # Unscaled for full variance comparison
